...
This report consists of four sections, not including the introduction. In @sec-data, we look at the data used for our report and include some tables and graphs to illustrate the different groups of people in our data. In @sec-model, we build the model and discuss its justification and explanation. The results of our predictions is highlighted in @sec-result using tables and graphs. Lastly, discussions are conducted based on the findings, which addresses the voting prediction results based on race, region, and employment status. 
...
## Talk about mail-in ballots AND Trump's later reaction to losing
