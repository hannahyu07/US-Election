---
title: "My title"
subtitle: "My subtitle if needed"
author: 
  - First author
  - Another author
thanks: "Code and data are available at: https://github.com/hannahyu07/US-Election"
date: today
date-format: long
abstract: "First sentence. Second sentence. Third sentence. Fourth sentence."
format: pdf
number-sections: true
bibliography: references.bib
---

```{r}
#| include: false
#| warning: false
#| message: false

library(tidyverse)
library(palmerpenguins)
library(here)
library(rstanarm)
library(modelsummary)

```


# Introduction

You can and should cross-reference sections and sub-sections. We use @citeR and @rohan.

The remainder of this paper is structured as follows. @sec-data....



# Data {#sec-data}

Some of our data is of penguins (@fig-bills), from @palmerpenguins.

```{r}
# read in data
ces2020 <-  read_csv(here::here("data/analysis_data/cleaned_ces2020.csv"))
```

```{r}
# change column type to factor
ces2020 <-
  ces2020 |>
  mutate(
    voted_for = as_factor(voted_for),
    race = factor(
      race,
      levels = c(
        "White",
        "Black",
        "Hispanic",
        "Asian",
        "Native American",
        "Middle Eastern",
        "Two or more races",
        "Other"
      )
    ),
    region = factor(
      region,
      levels = c(
        "Northeast",
        "Midwest",
        "South",
        "West"
      )
    ),
    employ = factor(
      employ,
      levels = c(
        "Full-time",
        "Part-time",
        "Temporarily laid off",
        "Unemployed",
        "Retired",
        "Permanently disabled",
        "Homemaker",
        "Student",
        "Other"
      )
    )
  ) |>
  select(voted_for, race, region, employ)
```



```{r}
#| label: fig-race_region
#| fig-cap: The distribution of presidential preferences, by region and race
#| echo: false

ces2020 |>
  ggplot(aes(x = race, fill = voted_for)) +
  stat_count(position = "dodge") +
  facet_wrap(facets = vars(region)) +
  theme_minimal() +
  labs(
    x = "Race",
    y = "Number of respondents",
    fill = "Voted for"
  ) +
  coord_flip() +
  scale_fill_brewer(palette = "Set1") +
  theme(legend.position = "bottom")
```
```{r}
#| label: fig-race_employ
#| fig-cap: The distribution of presidential preferences, by region and employment status
#| echo: false

ces2020 |>
  ggplot(aes(x = employ, fill = voted_for)) +
  stat_count(position = "dodge") +
  facet_wrap(facets = vars(region)) +
  theme_minimal() +
  labs(
    x = "Employment status",
    y = "Number of respondents",
    fill = "Voted for"
  ) +
  coord_flip() +
  scale_fill_brewer(palette = "Set1") +
  theme(legend.position = "bottom")
```
```{r}
#| label: fig-race_employ_white
#| fig-cap: The distribution of presidential preferences, by region and employment status, only white people
#| echo: false

ces2020_selected_data <- ces2020[ces2020$race == "White", ]

ces2020_selected_data |>
  ggplot(aes(x = employ, fill = voted_for)) +
  stat_count(position = "dodge") +
  facet_wrap(facets = vars(region)) +
  theme_minimal() +
  labs(
    x = "Employment status",
    y = "Number of respondents",
    fill = "Voted for"
  ) +
  coord_flip() +
  scale_fill_brewer(palette = "Set1") +
  theme(legend.position = "bottom")
```

Talk more about it.

And also planes (@fig-planes). (You can change the height and width, but don't worry about doing that until you have finished every other aspect of the paper - Quarto will try to make it look nice and the defaults usually work well once you have enough text.)

```{r}
#| label: fig-planes
#| fig-cap: Relationship between wing length and width
#| echo: false
#| warning: false
#| message: false

# analysis_data <- read_csv(here::here("data/analysis_data/analysis_data.csv"))
# 
# analysis_data |> 
#   ggplot(aes(x = width, y = length)) +
#   geom_point(alpha = 0.8) +
#   theme_minimal() +
#   labs(x = "Wing width (mm)",
#        y = "Wing length (mm)")
```

Talk way more about it. 



# Model

The goal of our modelling strategy is twofold. Firstly,...

Here we briefly describe the Bayesian analysis model used to investigate... Background details and diagnostics are included in [Appendix -@sec-model-details].

## Model set-up

Define $y_i$ as the number of seconds that the plane remained aloft. Then $\beta_i$ is the wing width and $\gamma_i$ is the wing length, both measured in millimeters.  



\begin{align} 
y_i|\pi_i &\sim \mbox{Bern}(\pi_i) \\
\mbox{logit}(\pi_i) &= \alpha + \beta_1 \times \mbox{race}_i + \beta_2 \times \mbox{region}_i + \beta_3 \times \mbox{employ}_i\\
\alpha &\sim \mbox{Normal}(0, 2.5) \\
\beta_1 &\sim \mbox{Normal}(0, 2.5) \\
\beta_2 &\sim \mbox{Normal}(0, 2.5)
\end{align}


We run the model in R [@citeR] using the `rstanarm` package of @rstanarm. We use the default priors from `rstanarm`.


### Model justification

We expect a positive relationship between the size of the wings and time spent aloft. In particular...

We can use maths by including latex between dollar signs, for instance $\theta$.


# Results

Our results are summarized in @tbl-modelresults.

```{r}
#| echo: false
#| eval: true
#| warning: false
#| message: false

political_preferences <-
  readRDS(file = here::here("models/political_preferences.rds"))

political_preferences2 <-
  readRDS(file = here::here("models/political_preferences2.rds"))


political_preferences3 <-
  readRDS(file = here::here("models/political_preferences3.rds"))


political_preferences4 <-
  readRDS(file = here::here("models/political_preferences4.rds"))

political_preferences5 <-
  readRDS(file = here::here("models/political_preferences5.rds"))

political_preferences6 <-
  readRDS(file = here::here("models/political_preferences6.rds"))

political_preferences7 <-
  readRDS(file = here::here("models/political_preferences7.rds"))

political_preferences8 <-
  readRDS(file = here::here("models/political_preferences8.rds"))

political_preferences9 <-
  readRDS(file = here::here("models/political_preferences9.rds"))
```


```{r}
#| echo: false
#| eval: true
#| label: tbl-modelresults1
#| tbl-cap: "Explanatory models Political Preferences (n = 40000)"
#| warning: false



modelsummary(
  list(
    "Support Biden" = political_preferences
  ),
  statistic = "mad"
  )


```


```{r}
#| echo: false
#| eval: true
#| label: tbl-modelresults2
#| tbl-cap: ""Explanatory models Political Preferences (n = 1000)"
#| warning: false



modelsummary(
  list(
    "Support Biden" = political_preferences2
  ),
  statistic = "mad"
  )


```

```{r}
modelsummary(
  list(
    "Support Biden" = political_preferences3
  ),
  statistic = "mad"
  )
```

```{r}
modelsummary(
  list(
    "Support Biden" = political_preferences4
  ),
  statistic = "mad"
  )
```
```{r}
modelsummary(
  list(
    "Support Biden" = political_preferences5
  ),
  statistic = "mad"
  )
```
```{r}
modelsummary(
  list(
    "Support Biden" = political_preferences6
  ),
  statistic = "mad"
  )
```
```{r}
modelsummary(
  list(
    "Support Biden" = political_preferences7
  ),
  statistic = "mad"
  )
```
```{r}
modelsummary(
  list(
    "Support Biden" = political_preferences8
  ),
  statistic = "mad"
  )
```

```{r}
modelsummary(
  list(
    "Support Biden" = political_preferences9
  ),
  statistic = "mad"
  )
```


```{r}
modelplot(political_preferences9, conf_level = 0.9) +
  labs(x = "90 per cent credibility interval")
```

# Discussion

## First discussion point {#sec-first-point}

If my paper were 10 pages, then should be be at least 2.5 pages. The discussion is a chance to show off what you know and what you learnt from all this. 

## Second discussion point

## Third discussion point

## Weaknesses and next steps

Weaknesses and next steps should also be included.

\newpage

\appendix

# Appendix {-}


# Additional data details

# Model details {#sec-model-details}

## Posterior predictive check

In @fig-ppcheckandposteriorvsprior-1 we implement a posterior predictive check. This shows...

In @fig-ppcheckandposteriorvsprior-2 we compare the posterior with the prior. This shows... 

```{r}
#| eval: true
#| echo: false
#| message: false
#| warning: false
#| label: fig-ppcheckandposteriorvsprior
#| layout-ncol: 2
#| fig-cap: "Examining how the model fits, and is affected by, the data"
#| fig-subcap: ["Posterior prediction check", "Comparing the posterior with the prior"]
# 
# pp_check(first_model) +
#   theme_classic() +
#   theme(legend.position = "bottom")
# 
# posterior_vs_prior(first_model) +
#   theme_minimal() +
#   scale_color_brewer(palette = "Set1") +
#   theme(legend.position = "bottom") +
#   coord_flip()
```

## Diagnostics

@fig-stanareyouokay-1 is a trace plot. It shows... This suggests...

@fig-stanareyouokay-2 is a Rhat plot. It shows... This suggests...

```{r}
#| echo: false
#| eval: true
#| message: false
#| warning: false
#| label: fig-stanareyouokay
#| fig-cap: "Checking the convergence of the MCMC algorithm"
#| fig-subcap: ["Trace plot", "Rhat plot"]
#| layout-ncol: 2

# plot(first_model, "trace")
# 
# plot(first_model, "rhat")
```



\newpage


# References


